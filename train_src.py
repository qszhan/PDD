...
def cal_acc_oda(loader, netF, netB, netC, flag=False):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
        for i in range(len(loader)):
    ...
    if flag:
        aa = [str(np.round(i, 2)) for i in acc]
        acc_each_category = ' '.join(aa)
        return known_acc, np.mean(acc), unknown_acc, HOS, acc_each_category
    else:
        # return known_acc, all_acc, unknown_acc respectively
...
def train_source(args):
    ...
    while iter_num < max_iter:
        try:
            (inputs_source, inputs_source_augs), labels_source, _ = next(iter_source)
        except:
...
def test_target(args):
    ...
    if args.da == 'oda':
        if args.dset == 'VISDA-C':
            acc_os1, acc_os2, acc_unknown, HOS, acc_list = cal_acc_oda(dset_loaders['test'], netF, netB, netC, True)
            log_str = '\nTask: {}, all_acc:{:.2f}, known_acc:{:.2f}, unknown_acc: {:.2f}, ' \
                      'HOS: {:.2f}%'.format(args.name,  acc_os2, acc_os1, acc_unknown,
                                            HOS) + '\n' + acc_list
        else:
            acc_os1, acc_os2, acc_unknown, hos = cal_acc_oda(dset_loaders['test'], netF, netB, netC)
...
if __name__ == "__main__":
    ...
    args.output_dir_src = osp.join(args.output, args.da, args.dset, names[args.s][0].upper()+ "cla_num" + str(args.class_num))
