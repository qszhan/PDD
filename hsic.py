...
def hsic_normalized(x, y, sigma=None, use_cuda=True, to_numpy=True):
    ...
    Px = torch.sqrt(torch.clamp(hsic_regular(x, x, sigma, use_cuda), min=1e-8))
    Py = torch.sqrt(torch.clamp(hsic_regular(y, y, sigma, use_cuda), min=1e-8))
    # print("Px:", Px)
    # print("Py:", Py)
    # print("Pxy:", Pxy)
    # Px = torch.sqrt(hsic_regular(x, x, sigma, use_cuda))
    # Py = torch.sqrt(hsic_regular(y, y, sigma, use_cuda))
    thehsic = torch.where(Pxy == 0, torch.tensor(0.0), Pxy / (Px * Py + 1e-8))
