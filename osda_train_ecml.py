...
from utils import make_dataset, data_load_list, identify_unkown_idx
import random
from scipy.spatial.distance import cdist
from sklearn.metrics import confusion_matrix
from sam import SAM
...
def cal_unknown_in_fea_batch(netC, feas_all, unknown_mask):
    fc_weights = netC.state_dict()['fc.weight_v']
    fc_bias = netC.state_dict()['fc.bias']
    bf_fea = -np.matmul(pinv(fc_weights.cpu().numpy()), fc_bias.cpu().numpy())
    feature_id_val = feas_all[~unknown_mask]
    if feature_id_val.shape[-1] >= 2048:
        DIM = 1000
    elif feature_id_val.shape[-1] >= 768:
        DIM = 512
    ...
    ec.fit(feature_id_val - bf_fea)
    eig_vals, eigen_vectors = np.linalg.eig(ec.covariance_)
    NS = np.ascontiguousarray((eigen_vectors.T[np.argsort(eig_vals * -1)[DIM:]]).T)  # (256, 128)
    residual_batch = -norm(np.matmul(feas_all[unknown_mask] - bf_fea, NS), axis=-1)
    return torch.from_numpy(residual_batch)
